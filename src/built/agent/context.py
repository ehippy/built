    ...
    *,
    ...
    if project.test_command:
        test_gate = (
            ...
            "command, byte for byte — not piped into `grep`/`tail`/anything else, and not chained "
            "with `&&`/`;` onto another command in the same call, even a file edit that itself "
            "changed nothing meaningful. Either of those makes the check fail even though the test "
            "command did in fact pass, and it will keep failing on every retry until you run it "
            "bare. If the suite's output is too large to want in full, redirect it to a file first "
            f"(`{project.test_command} > /tmp/test_output.txt 2>&1`) and grep that file in a "
            "separate, later bash call — a read-only follow-up like that doesn't touch tracked "
            "files, so it won't invalidate the run. You must also not write_file/edit_file/bash "
            "anything that changes files afterward without rerunning it, or submit_for_test will be "
            "rejected.\n\n"
        )
    else:
        test_gate = (
    ...
    *,
    ...
    if project.test_command:
        test_gate = (
            ...
            "command, byte for byte — not piped into `grep`/`tail`/anything else, and not chained "
            "with `&&`/`;` onto another command, even if the pipeline as a whole exits 0. If the "
            "output is too large to want in full, redirect it to a file bare "
            f"(`{project.test_command} > /tmp/test_output.txt 2>&1`) and grep that file separately "
            "afterward — that read-only follow-up doesn't invalidate the run. It must have exited "
            "0, and you must not write_file/edit_file/bash anything afterward without rerunning it "
            "— a green run followed by an unverified tweak doesn't count as tested. If something "
            "is wrong, call request_changes — see below for what the feedback needs to actually "
            "contain.\n\n"
        )
    else:
        test_gate = (
