...
def build_chat_prompt(
    ...
    *,
    ...
    system = (
        ...
        "references a card by name or description); create_ticket/update_ticket to file or edit "
        "cards directly once an idea is concrete — use them proactively once the conversation reaches "
        "a real, well-scoped decision, rather than waiting to be asked explicitly; and "
        "unblock_card/archive_card/unarchive_card for board maintenance the human explicitly asks "
        "for — unblocking a blocked/failed card (a human-initiated retry, not gated by the Reviver's "
        "auto-retry budget), or archiving/unarchiving one to take it off the board or bring it back. "
        "A created card "
    ...
    *,
    ...
    if project.test_command:
        test_gate = (
            ...
            "is checked server-side against your most recent bash run, and the test command must be "
            f"the last thing that call runs: `{project.test_command}` on its own is fine, as is "
            f"chaining something before it (`some-command && {project.test_command}`), as is piping "
            f"it into something else to trim a large suite's output (`{project.test_command} | grep "
            "...`) — piping is checked against the test command's own exit code specifically, not "
            "whatever the last stage of the pipe returns, so it can't be used to launder a real "
            f"failure into a green result. What doesn't work: the test command anywhere but last "
            f"(`{project.test_command} && something-else`, `something | {project.test_command}`), or "
            "any of `||`, backgrounding (`&`), subshells, or command substitution anywhere in the "
            "call — those aren't verifiable and the check will just reject the call outright. You "
            "must also not write_file/edit_file/bash anything that changes files afterward without "
            "rerunning it, or submit_for_test will be rejected.\n\n"
        )
    else:
        test_gate = (
    ...
    *,
    ...
    if project.test_command:
        test_gate = (
            ...
            "checked server-side against your most recent bash run, and the test command must be "
            f"the last thing that call runs: `{project.test_command}` alone is fine, as is chaining "
            f"something before it, as is piping it into `grep`/`tail`/etc. to trim a large suite's "
            "output — piping is checked against the test command's own exit code specifically, not "
            "whatever the pipe's last stage returns, so it can't launder a real failure into a green "
            f"result. What doesn't work: the test command anywhere but last, or any of `||`, "
            "backgrounding, subshells, or command substitution in the call — those get rejected "
            "outright as unverifiable. It must have exited 0, and you must not write_file/edit_file/"
            "bash anything afterward without rerunning it — a green run followed by an unverified "
            "tweak doesn't count as tested. If something is wrong, call request_changes — see below "
            "for what the feedback needs to actually contain.\n\n"
        )
    else:
        test_gate = (
